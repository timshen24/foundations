...
import exercises.errorhandling.NEL._
...
object ValidationExercises {
  ...
  def validateCountry(countryCode: String): Validation[FormError, Country] =
    if (countryCode.length >= 3 && countryCode.forall(c => c.isLetter && c.isUpper))
      Country.all.find(_.code == countryCode).toValid(NotSupported(countryCode))
    else
      Invalid(one(InvalidFormat(countryCode)))
  ...
  def checkUsernameCharacters(username: String): Validation[InvalidCharacters, Unit] = {
    ...
    Validation.cond(errList.isEmpty, Right(()), InvalidCharacters(errList))
  }
